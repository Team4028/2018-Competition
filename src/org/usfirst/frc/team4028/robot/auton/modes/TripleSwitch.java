...
public class TripleSwitch extends AutonBase {
	...
	Path fromSwitchToFrontOfPyramidPath, toPyramid, fromPyramid, toSwitchAfterSTurn; // Cube 2
	...
	public TripleSwitch(boolean isLeftSwitch) {
		if (isLeftSwitch) {
			...
			toSwitchAfterSTurn = Paths.getPath(PATHS.S_TURN_TO_L_SWITCH);
			...
			toSwitchAfterSTurn = Paths.getPath(PATHS.S_TURN_TO_R_SWITCH);
	...
	public void routine() {	
		...
		runAction(new RunMotionProfileAction(toPyramid));
